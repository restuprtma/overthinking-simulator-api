#!/usr/bin/env bash
# =============================================================================
# orderbook-consumer — build, validate, deploy automation
# =============================================================================
# Flow:
#   1. Validate prereqs (kubectl, git, kubeconfig, namespace, pull secret)
#   2. Ensure clean tree on production branch
#   3. Push to origin/production (Docker Hub Autobuild webhook fires)
...
#   7. Tail post-deploy logs and surface consumer startup info
#
# Prerequisites:
#   - Docker Hub Automated Build configured for
#       venturoid/tuai-orderbook-consumer-production
...
#   - Secret tuai-be-orderbook-consumer-env in namespace tuai (runtime env vars)
#
# Usage:
...
IMAGE="venturoid/tuai-orderbook-consumer-production"
NAMESPACE="tuai"
DEPLOYMENT="tuai-be-orderbook-consumer"
BRANCH="production"
...
Prereqs:
  - Docker Hub Automated Build for venturoid/tuai-orderbook-consumer-production
...
kubectl get ns "$NAMESPACE" >/dev/null 2>&1 || { err "namespace $NAMESPACE not reachable"; exit 1; }
...
echo
echo "  --- recent log ---"
kubectl logs -n "$NAMESPACE" "$POD_NAME" --tail=20 2>/dev/null || warn "could not fetch logs yet"

STARTUP=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
  | grep -m1 'orderbook-consumer starting' || echo "")
echo
if [ -n "$STARTUP" ]; then
  ok "consumer started: $STARTUP"
else
...
echo
dim "next steps (validate engine output during market hours):"
dim "  redis-cli -n 9 HGETALL orderbook:BBCA:_meta"
dim "  redis-cli -n 9 HGETALL orderbook:BBCA:bid"
dim "  nats sub 'idx.orderbook.delta.BBCA' --count=20"
