#!/usr/bin/env bash
# =============================================================================
# resend-trade-consumer — build, validate, deploy automation
# =============================================================================
# Flow:
...
#
# Prerequisites:
#   - Docker Hub Automated Build configured for venturoid/tuai-be-resend-trade-consumer
#     with rules:  Branch=production → Tag=<sourceref|literal>
#                  Dockerfile=deployments/docker/resend-trade-consumer.Dockerfile
#                  Build Context=/
#   - dockerhub-secret-venturoid Secret exists in namespace tuai (image pull)
#   - resend-trade-consumer-env Secret exists in namespace tuai with required env vars
#
# Usage:
...
IMAGE="venturoid/tuai-be-resend-trade-consumer"
NAMESPACE="tuai"
DEPLOYMENT="resend-trade-consumer"
BRANCH="production"
...
Prereqs:
  ...
  - Secret resend-trade-consumer-env in namespace tuai (runtime env vars)
...
STARTUP=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
  | grep -m1 'resend-trade-consumer starting' || echo "")
echo
