#!/usr/bin/env bash
# =============================================================================
# tradedone-consumer — build, validate, deploy automation
# =============================================================================
# Flow:
...
#
# Prerequisites:
#   - Docker Hub Automated Build configured for venturoid/tuai-be-tradedone-consumer
#     with rules:  Branch=production → Tag=<sourceref|literal>
#                  Dockerfile=deployments/docker/tradedone-consumer.Dockerfile
#                  Build Context=/
#   - dockerhub-secret-venturoid Secret exists in namespace tuai (image pull)
#   - tuai-be-tradedone-consumer-env Secret exists in namespace tuai with required env vars
#
# Usage:
...
IMAGE="venturoid/tuai-be-tradedone-consumer"
NAMESPACE="tuai"
DEPLOYMENT="tuai-be-tradedone-consumer"
BRANCH="production"
...
Prereqs:
  ...
  - Secret tuai-be-tradedone-consumer-env in namespace tuai (runtime env vars)
...
STARTUP=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
  | grep -m1 'tradedone-consumer starting' || echo "")
echo
