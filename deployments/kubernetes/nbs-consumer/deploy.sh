#!/usr/bin/env bash
# =============================================================================
# nbs-consumer — build, validate, deploy automation
# =============================================================================
# Flow:
...
#   7. Tail post-deploy logs and verify QuestDB sink is configured
#
# Prerequisites:
#   - Docker Hub Automated Build configured for venturoid/tuai-be-nbs-consumer
#     with rules:  Branch=production → Tag=<sourceref|literal>
#                  Dockerfile=deployments/docker/nbs-consumer.Dockerfile
#                  Build Context=/
#   - dockerhub-secret-venturoid Secret exists in namespace tuai (image pull)
#   - tuai-be-nbs-consumer-env Secret exists in namespace tuai
#   - QuestDB tables `nbs_stock` and `nbs_broker` already created
#     (see README.md §"Tables required in QuestDB")
#
# Usage:
...
IMAGE="venturoid/tuai-be-nbs-consumer"
NAMESPACE="tuai"
...
Flow:
  ...
  5. Bump tag in deployment.yaml
  6. kubectl apply + rollout status
  7. Tail post-deploy logs and verify QuestDB sink configured
...
Prereqs:
  ...
  - Secret tuai-be-nbs-consumer-env in namespace tuai (runtime env vars)
  - QuestDB tables nbs_stock + nbs_broker exist (see README §"Tables required")
...
# Verify QuestDB sink configured in startup log
STARTUP=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
  | grep -m1 'nbs-consumer starting' || echo "")
echo
if echo "$STARTUP" | grep -q "questdb_addr"; then
  ok "questdb sink configured ($(echo "$STARTUP" | grep -oE 'questdb_addr=[^ ]+' || true), $(echo "$STARTUP" | grep -oE 'questdb_stock_table=[^ ]+' || true), $(echo "$STARTUP" | grep -oE 'questdb_broker_table=[^ ]+' || true))"
else
