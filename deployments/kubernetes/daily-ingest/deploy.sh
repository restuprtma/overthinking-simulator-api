#!/usr/bin/env bash
# =============================================================================
# daily-ingest — build, validate, deploy automation
# =============================================================================
# Flow:
...
#   7. Tail post-deploy logs and verify daemon scheduled correctly
#
# Prerequisites:
#   - Docker Hub Automated Build configured for venturoid/tuai-be-daily-ingest
#     with rules:  Branch=production → Tag=<sourceref|literal>
#                  Dockerfile=deployments/docker/daily-ingest.Dockerfile
#                  Build Context=/
#   - dockerhub-secret-venturoid Secret exists in namespace tuai (image pull)
#   - tuai-be-daily-ingest-env Secret exists in namespace tuai (runtime env)
#
# Usage:
...
IMAGE="venturoid/tuai-be-daily-ingest"
NAMESPACE="tuai"
DEPLOYMENT="tuai-be-daily-ingest"
BRANCH="production"
...
Flow:
  ...
  5. Bump tag in deployment.yaml
  6. kubectl apply + rollout status
  7. Tail post-deploy logs and verify daemon scheduled
...
Prereqs:
  ...
  - Secret tuai-be-daily-ingest-env in namespace tuai (runtime env vars)
...
# Verify daemon scheduled successfully
STARTUP=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
  | grep -m1 'daily-ingest daemon starting' || echo "")
echo
if [ -n "$STARTUP" ]; then
  ok "daemon started ($(echo "$STARTUP" | grep -oE 'target=[^ ]+' || true), $(echo "$STARTUP" | grep -oE 'run_at=[^ ]+' || true))"
  NEXT=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
    | grep -m1 'daily-ingest daemon scheduled' || echo "")
  if [ -n "$NEXT" ]; then
    dim "$(echo "$NEXT" | grep -oE 'next_fire_wib=[^ ]+' || true)"
  fi
else
