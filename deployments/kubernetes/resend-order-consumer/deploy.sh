#!/usr/bin/env bash
# =============================================================================
# resend-order-consumer — build, validate, deploy automation
# =============================================================================
# Flow:
...
#
# Prerequisites:
#   - Docker Hub Automated Build configured for venturoid/tuai-be-resend-order-consumer
#     with rules:  Branch=production → Tag=<sourceref|literal>
#                  Dockerfile=deployments/docker/resend-order-consumer.Dockerfile
#                  Build Context=/
#   - dockerhub-secret-venturoid Secret exists in namespace tuai (image pull)
#   - resend-order-consumer-env Secret exists in namespace tuai with required env vars
#
# Usage:
...
IMAGE="venturoid/tuai-be-resend-order-consumer"
NAMESPACE="tuai"
DEPLOYMENT="resend-order-consumer"
BRANCH="production"
...
Prereqs:
  ...
  - Secret resend-order-consumer-env in namespace tuai (runtime env vars)
...
STARTUP=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
  | grep -m1 'resend-order-consumer starting' || echo "")
echo
