#!/usr/bin/env bash
# =============================================================================
# running-trade-consumer — build, validate, deploy automation
# =============================================================================
# Flow:
...
#   7. Tail post-deploy logs and verify QuestDB sink + aggregator config
#
# Prerequisites:
#   - Docker Hub Automated Build configured for venturoid/tuai-be-running-trade-consumer
#     with rules:  Branch=production → Tag=<sourceref|literal>
#                  Dockerfile=deployments/docker/running-trade-consumer.Dockerfile
#                  Build Context=/
#   - dockerhub-secret-venturoid Secret exists in namespace tuai (image pull)
#   - tuai-be-running-trade-consumer-env Secret exists in namespace tuai
#
# Usage:
...
IMAGE="venturoid/tuai-be-running-trade-consumer"
NAMESPACE="tuai"
DEPLOYMENT="tuai-be-running-trade-consumer"
BRANCH="production"
...
Prereqs:
  ...
  - Secret tuai-be-running-trade-consumer-env in namespace tuai (runtime env vars)
...
# Verify QuestDB sink + timeframes from startup log
STARTUP=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
  | grep -m1 'running-trade-consumer starting' || echo "")
echo
if echo "$STARTUP" | grep -q "questdb_addr"; then
  ok "questdb sink configured ($(echo "$STARTUP" | grep -oE 'questdb_table=[^ ]+' || true))"
  if echo "$STARTUP" | grep -q "timeframes"; then
    dim "timeframes=$(echo "$STARTUP" | grep -oE 'timeframes=\[[^]]+\]' || true)"
  fi
else
