#!/usr/bin/env bash
# =============================================================================
# quote-consumer — build, validate, deploy automation
# =============================================================================
# Flow:
...
#   7. Tail post-deploy logs and verify Redis sink is configured
#
# Prerequisites:
#   - Docker Hub Automated Build configured for venturoid/tuai-be-quote-consumer
#     with rules:  Branch=production → Tag=<sourceref|literal>
#                  Dockerfile=deployments/docker/quote-consumer.Dockerfile
#                  Build Context=/
#   - dockerhub-secret-venturoid Secret exists in namespace tuai (image pull)
#   - tuai-be-quote-consumer-env Secret exists in namespace tuai
#
# Usage:
...
IMAGE="venturoid/tuai-be-quote-consumer"
NAMESPACE="tuai"
DEPLOYMENT="tuai-be-quote-consumer"
BRANCH="production"
...
Flow:
  ...
  5. Bump tag in deployment.yaml
  6. kubectl apply + rollout status
  7. Tail post-deploy logs and verify Redis sink configured
...
Prereqs:
  ...
  - Secret tuai-be-quote-consumer-env in namespace tuai (runtime env vars)
...
# Verify Redis sink configured in startup log
STARTUP=$(kubectl logs -n "$NAMESPACE" "$POD_NAME" 2>/dev/null \
  | grep -m1 'quote-consumer starting' || echo "")
echo
if echo "$STARTUP" | grep -q "redis_addr"; then
  ok "redis sink configured ($(echo "$STARTUP" | grep -oE 'redis_addr=[^ ]+' || true), $(echo "$STARTUP" | grep -oE 'redis_db=[^ ]+' || true))"
else
