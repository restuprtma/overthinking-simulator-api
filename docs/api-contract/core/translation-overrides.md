...
- Setiap client punya **slug** DNS-safe unik (contoh `acme`, `wizhub-demo`) yang dipakai FE untuk bootstrap tanpa login
...
- **Slug discovery** — FE harus tahu slug client-nya sebelum bootstrap. Source umum: subdomain (`acme.wizhub.id` → `acme`), build-time env var, atau user preference di localStorage.
