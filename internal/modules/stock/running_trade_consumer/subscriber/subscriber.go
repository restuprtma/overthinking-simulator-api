// Package subscriber owns the NATS JetStream connection + durable consumer
// for the OHLCV aggregator. It hands every Trade message to the aggregator
// and ack/nak based on the handler return.
package subscriber
...
// Handler processes one decoded envelope. Return error → message NAK
// (JetStream redelivers per stream policy). Return nil → ack.
type Handler func(ctx context.Context, env iqplus_envelope.Envelope) error

// Config contains everything needed to connect to NATS and subscribe.
type Config struct {
	URL            string
	Token          string
	ClientName     string
	Stream         string        // e.g. IDX_TICK
	DurableName    string        // e.g. running-trade-consumer
	FilterSubject  string        // e.g. idx.trade.>
	AckWait        time.Duration // server-side redelivery window
	MaxAckPending  int           // back-pressure cap
...
func DefaultConfig() Config {
	return Config{
		...
		Stream:         "IDX_TICK",
		DurableName:    "running-trade-consumer",
		FilterSubject:  "idx.trade.>",
...
type Stats struct {
	...
	Acked    uint64
	Naked    uint64
	Errors   uint64 // envelope decode errors
}
...
func New(cfg Config, h Handler) *Subscriber {
	...
	if cfg.ClientName == "" {
		cfg.ClientName = "running-trade-consumer"
	}
	return &Subscriber{cfg: cfg, handler: h}
}

// Start dials NATS, creates/updates the durable consumer, and begins
// pushing messages through the handler. Non-blocking: returns once the
// consumer is registered. Use Close() to stop.
func (s *Subscriber) Start(ctx context.Context) error {
	...
	logger.Log.Info("running-trade subscriber ready",
...
func (s *Subscriber) dispatch(ctx context.Context, msg jetstream.Msg) {
	...
	if err != nil {
		...
		// Bad envelope is not retry-able — ack to avoid hot redelivery loop.
		_ = msg.Ack()
		return
	}
	...
	if err := s.handler(ctx, env); err != nil {
		...
		// Surface ack errors at debug — handler already logged the failure.
...
// Close stops consuming and drains the NATS connection. Safe to call once.
