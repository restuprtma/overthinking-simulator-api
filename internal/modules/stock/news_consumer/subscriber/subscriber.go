...
// consumer for news. (TODO: extract to shared `iqplus_subscriber`
// once the 5th consumer pattern stabilizes — currently 4 copies.)
package subscriber
...
type Config struct {
	...
	Token          string
	ClientName     string
	Stream         string // e.g. IDX_NEWS
	DurableName    string // e.g. news-indexer
	FilterSubject  string // e.g. idx.news.>
...
// DefaultConfig fills the safe operational knobs for News consumption.
func DefaultConfig() Config {
	return Config{
		ClientName:     "news-consumer",
		Stream:         "IDX_NEWS",
		DurableName:    "news-indexer",
		FilterSubject:  "idx.news.>",
		AckWait:        60 * time.Second, // longer — Mongo write is slower than Redis
		MaxAckPending:  500,              // news rate is low, no need for big window
...
func (s *Subscriber) Start(ctx context.Context) error {
	...
	logger.Log.Info("news subscriber ready",
