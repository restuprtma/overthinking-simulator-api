...
// consumer for resend orders.
//
// (TODO: 7 nearly-identical subscribers across modules — extract to
...
type Config struct {
	...
	DurableName    string // e.g. resend-order-backfill
	FilterSubject  string // e.g. idx.resend.order.>
...
// DefaultConfig fills the safe operational knobs for resend-order backfill.
func DefaultConfig() Config {
	return Config{
		ClientName:     "resend-order-consumer",
		Stream:         "IDX_TICK",
		DurableName:    "resend-order-backfill",
		FilterSubject:  "idx.resend.order.>",
...
func New(cfg Config, h Handler) *Subscriber {
	...
	if cfg.ClientName == "" {
		cfg.ClientName = "resend-order-consumer"
	}
...
func (s *Subscriber) Start(ctx context.Context) error {
	...
	logger.Log.Info("resend-order subscriber ready",
