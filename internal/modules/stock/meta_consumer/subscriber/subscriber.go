...
// consumer for the meta-consumer. Difference from running-trade/quote: uses
// FilterSubjects (plural, server-side OR) so one consumer can pull
// multiple subject prefixes from a single stream.
//
// (TODO: this duplicates running-trade/quote subscribers — extract to shared
// `iqplus_subscriber` once a 4th consumer is added.)
package subscriber
...
type Config struct {
	...
	Token          string
	ClientName     string
	Stream         string   // e.g. IDX_META
	DurableName    string   // e.g. meta-state-cache
	FilterSubjects []string // e.g. {"idx.status.>","idx.activity.>",...}
...
// DefaultConfig fills the safe operational knobs for meta consumption.
func DefaultConfig() Config {
	return Config{
		ClientName:  "meta-consumer",
		Stream:      "IDX_META",
		DurableName: "meta-state-cache",
...
// Start dials NATS, creates/updates the durable consumer with multi-filter.
func (s *Subscriber) Start(ctx context.Context) error {
	...
	logger.Log.Info("meta subscriber ready",
