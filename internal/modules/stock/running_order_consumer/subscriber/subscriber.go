...
// consumer for live Type 16 (Order) events.
//
// (TODO: 8 nearly-identical subscribers across modules — extract to
...
type Config struct {
	...
	DurableName    string // e.g. running-order-consumer
	FilterSubject  string // e.g. idx.order.>
...
// DefaultConfig fills the safe operational knobs for live order ingest.
func DefaultConfig() Config {
	return Config{
		ClientName:     "running-order-consumer",
		Stream:         "IDX_TICK",
		DurableName:    "running-order-consumer",
		FilterSubject:  "idx.order.>",
...
func New(cfg Config, h Handler) *Subscriber {
	...
	if cfg.ClientName == "" {
		cfg.ClientName = "running-order-consumer"
	}
...
func (s *Subscriber) Start(ctx context.Context) error {
	...
	logger.Log.Info("running-order subscriber ready",
