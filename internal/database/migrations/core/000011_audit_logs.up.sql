...
-- =====================================================
-- Migration 011: Per-entity audit trail reusable by all features.
--
-- Design notes:
--   - Polymorphic (reff_type, reff_id) — schema-qualified so entries
--     can span multiple schemas (finance, rental, ...) without name
--     collisions.
...
CREATE TABLE IF NOT EXISTS core.audit_logs (
    ...
    -- Extra context (reason, attachment name, correlation ids, ...)
    metadata JSONB,
...
COMMENT ON COLUMN core.audit_logs.action IS
    'Event kind: created, updated, cancelled, posted, voided, deleted, restored, attachment_added, attachment_removed, ... — snake_case, extensible (validated by regex, not CHECK IN, so new modules can add actions without migration).';
...
COMMENT ON COLUMN core.audit_logs.metadata IS
    'Free-form context for the event: comment, reason, filename, correlation ids, etc.';
