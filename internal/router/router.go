...
func Setup(router *gin.Engine, db *pgxpool.Pool, cfg *config.Config) {
	...
	router.Use(cors.New(cors.Config{
		AllowOrigins:     []string{"http://localhost:5173", "http://localhost:3000", "http://localhost:3001", "http://localhost:8081", "https://app.wizhub.id", "https://jesuit.venturo.pro", "https://jesuit-v2.venturo.pro"},
	...
	coreV1 := router.Group("/core/v1")
	{
		...
		// narrow branch-scoped reads to the caller's user_branches set.
