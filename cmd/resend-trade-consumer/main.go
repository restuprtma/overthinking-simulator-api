// Command resend-trade-consumer subscribes to idx.resend.trade.> on JetStream
// and writes every Type 27 row into the QuestDB `trades` table.
//
// Build for FreeBSD:
//
//	GOOS=freebsd GOARCH=amd64 go build -o bin/resend-trade-consumer-freebsd-amd64 ./cmd/resend-trade-consumer
//
// Required environment variables:
//
//	NATS_URL                e.g. nats://10.10.8.2:4222
//	QUESTDB_ADDRESS         host:port (HTTP ILP, e.g. 10.10.8.51:9000)
//	QUESTDB_TABLE           destination table (e.g. trades)
//
...
//	NATS_DURABLE            default resend-trade-backfill
//	NATS_FILTER_SUBJECT     default idx.resend.trade.>
...
import (
	...
	resend_trade_consumer "tuai/internal/modules/stock/resend_trade_consumer"
...
func main() {
	...
	logger.Log.Info("resend-trade-consumer starting",
	...
	mod, err := resend_trade_consumer.Initialize(ctx, cfg)
	if err != nil {
	...
	logger.Log.Info("resend-trade-consumer stopped cleanly")
}

func loadConfig() (resend_trade_consumer.Config, error) {
	...
	qdb := tradesink.DefaultQuestDBConfig()
	...
	return resend_trade_consumer.Config{
		Subscriber: sub,
		QuestDB:    qdb,
		Service:    svcCfg,
	}, nil
...
//
// Lookup order:
//  1. $RESEND_TRADE_CONSUMER_ENV_FILE
//  2. ./cmd/resend-trade-consumer/.env
//  3. <exe-dir>/.env
//  4. <exe-dir>/resend-trade-consumer.env
func loadEnvFile() {
	if explicit := os.Getenv("RESEND_TRADE_CONSUMER_ENV_FILE"); explicit != "" {
		if err := godotenv.Load(explicit); err != nil {
			fmt.Fprintf(os.Stderr,
				"failed to load RESEND_TRADE_CONSUMER_ENV_FILE=%s: %v\n",
				explicit, err)
			os.Exit(1)
		}
	...
	candidates := []string{"cmd/resend-trade-consumer/.env"}
	if exe, err := os.Executable(); err == nil {
		dir := filepath.Dir(exe)
		candidates = append(candidates,
			filepath.Join(dir, ".env"),
			filepath.Join(dir, "resend-trade-consumer.env"),
		)
	}
...
func defaultClientName() string {
	...
	return "resend-trade-consumer-" + host
}
